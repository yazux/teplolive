...
<div class="soc-enter">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		<div class="enter-aswho">Войти при помощи</div>
		<div class="bx-auth-services">
<?foreach($arParams["~AUTH_SERVICES"] as $service):?>
			<div><a href="javascript:void(0)" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><i class="bx-ss-icon <?=htmlspecialchars($service["ICON"])?>"></i></a></div>
<?endforeach?>
		</div>
