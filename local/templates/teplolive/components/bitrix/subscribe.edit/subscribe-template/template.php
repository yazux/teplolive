...
<div class="subscription">
	...
	<div class="subscription-form">
		...
		<table cellspacing="0" class="subscription-layout">
			<tr>
				<td class="field-name-2"><?echo GetMessage("CT_BSE_EMAIL_LABEL")?></td>
				<td class="field-form">
				...
				<td class="field-form">
					<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
						<div class="subscription-rubric">
							<span class="chetkaCheck"><input type="checkbox" id="RUBRIC_<?echo $itemID?>" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> /></span><label for="RUBRIC_<?echo $itemID?>"><b><?echo $itemValue["NAME"]?></b><span><?//echo $itemValue["DESCRIPTION"]?></span></label>
                        <!--<label for="RUBRIC_<?echo $itemID?>" onclick="" class='checkbox'></label>-->
                        </div>
					<?endforeach;?>
	...
	<div class="subscription-utility">
		...
		<input name="sf_EMAIL" type="text" class="subscription-textbox" value="<?echo GetMessage("CT_BSE_EMAIL")?>" onblur="if (this.value=='')this.value='<?echo GetMessage("CT_BSE_EMAIL")?>'" onclick="if (this.value=='<?echo GetMessage("CT_BSE_EMAIL")?>')this.value=''" /> <input class='vislat' type="submit" value="<?echo GetMessage("CT_BSE_BTN_SEND")?>" />
	</div>
	</form>
	<?endif?>
