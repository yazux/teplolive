...
foreach ($arResult['SECTIONS'] as $arPicture) {
    ...
    if (!empty($arParams['ALICORN'])){
        $luna["SECTION_ID"] = $arParams['ALICORN'];
    }
    else {
        //$luna['DEPTH_LEVEL'] = 2;
        $luna["SECTION_ID"] = $arPicture['ID'];
        /*$lich = CIBlockSection::GetList(Array(), $luna, false, $rasta);
        while($leoric = $lich->GetNextElement()){
            $ursa = $leoric->GetFields();
        }
        echo "<pre>";print_r($ursa);echo "</pre>";
        $luna["SECTION_ID"] = $ursa['ID'];*/
    }
    $lich = CIBlockSection::GetList(Array(), $luna, false, $rasta);
    while($leoric = $lich->GetNextElement())
    {
    ...
    //echo "<pre>"; print_r($ursa); echo "</pre>";
    foreach ($ursa as $ursaMinor) {
        //echo "<pre>"; print_r($ursaMinor); echo "</pre>";
        if ($ursaMinor['UF_HAS_ELEMENTS'] =='N') {
            $arResult['SECTIONS'][$sectionsCounter]['MINOR_SECTIONS'][] = $ursaMinor;
        }
        else {
            $IdForSectionList[] = $ursaMinor;
        }
    }
    unset ($ursa);
    unset ($ursaMinor);
    GLOBAL $IdForSectionList;
