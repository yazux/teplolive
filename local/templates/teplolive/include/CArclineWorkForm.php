...
?>
<form id='callForm' action="/callmeplz/index.php?AJAX=2&knopka=1" method="post">
	<input name="form_type" type="hidden" value="<?=$this->razdel?>">
	<input name="name" type="text" placeholder='Ф.И.О.' id='inpName' class="inp1">
	<input name="time" type="text" placeholder='дата и время' id='inpTime' class="inp1">
    <input name="phone" type="text" placeholder='телефон' id='inpPhone' class="inp1">
 	<input name="knopka" type="submit" value='' class="input_submit2">
</form>
<?
}
...
if ((intval($_REQUEST["form_type"])==$this->razdel) && ($error == null)):
    if(intval($_REQUEST["knopka"])<>1){die();}
	$el = new CIBlockElement;
	$preview_text='<p>Ф.И.О.: '.htmlspecialchars($_REQUEST["name"]).'</p>';
	$preview_text.='<p>Время звонка: '.htmlspecialchars($_REQUEST["time"]).'</p>';
 ...
 if ($PRODUCT_ID>0)
 {
	...
	$arFields = array(
		...
		"FIELD_2"    => htmlspecialchars($_REQUEST["time"]),
		"FIELD_3"    => htmlspecialchars($_REQUEST["phone"]),
		"FIELD_4"    => $_SERVER["REMOTE_ADDR"]         
    );    
