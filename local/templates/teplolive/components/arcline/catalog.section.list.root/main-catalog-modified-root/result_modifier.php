...
foreach ($arResult['SECTIONS'] as $secKey => $arPicture) {
    $ursa = array();
    $leoric = array();
    $rasta = array(
        "UF_*",
    ...
    foreach ($ursa as $ursaMinor) {
        if ($ursaMinor['UF_HAS_ELEMENTS'] =='N') {
            $arResult['SECTIONS'][$secKey]['MINOR_SECTIONS'][] = $ursaMinor;
        }
    }
    ...
    if (!empty($arPicture['PREVIEW_PICTURE'])) {
        ...
        foreach ($arSmall as $arPreview){
            if (!empty($arPreview['PREVIEW_PICTURE'])){
                $arResult['SECTIONS'][$secKey]['PREVIEW_PICTURE'] = CFile::GetPath($arPreview['PREVIEW_PICTURE']);
                break;
            }
            $sectionsItemCounter++;
        }
