...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	?>
	<div class="news-items" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
...
?>    <div class='news-date'>
     ...
     <div class='news-float'>
	  <?if (strlen($arItem['DISPLAY_PROPERTIES']['FILE']['FILE_VALUE']['SRC']) > 0):?>
      <div class='download'><a <?if ($USER->IsAuthorized()):?>href='<?=$arItem['DISPLAY_PROPERTIES']['FILE']['FILE_VALUE']['SRC'];?>'<?else:?>rel="#overlay" href="/auth/login.php"<?endif;?>>скачать</a></div>
    <?endif;?>
  	<?if($arItem["NAME"]):?>
